...
func serveFile(w http.ResponseWriter, r *http.Request, fs http.FileSystem, name string, redirect bool) {
	//var a []string=[]string{"aa","bbb"}
	var indexPage = []string{"/index.html", "/index.htm"}

	// redirect .../index.html to .../
	// can't use Redirect() because that would make the path absolute,
	// which would be a problem running under StripPrefix
	for _, indexp := range indexPage {
		if strings.HasSuffix(r.URL.Path, indexp) {
			localRedirect(w, r, "./")
			return
		}
	}
	...
	if d.IsDir() {
		...
		for _, indexp := range indexPage {
			index := strings.TrimSuffix(name, "/") + indexp
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) {
	...
	if size >= 0 {
		...
		if Headerget(w.Header(), "Content-Encoding") == "" {
...
func checkPreconditions(w http.ResponseWriter, r *http.Request, modtime time.Time) (done bool, rangeHeader string) {
	...
	rangeHeader = Headerget(r.Header, "Range")
...
func checkIfRange(w http.ResponseWriter, r *http.Request, modtime time.Time) condResult {
	...
	ir := Headerget(r.Header, "If-Range")
	if ir == "" {
		return condNone
	}
	etag, _ := scanETag(ir)
	if etag != "" {
		if etagStrongMatch(etag, Headerget(w.Header(), "Etag")) {
			return condTrue
		} else {
			return condFalse
		}
...
func checkIfNoneMatch(w http.ResponseWriter, r *http.Request) condResult {
	inm := Headerget(r.Header, "If-None-Match")
	if inm == "" {
		return condNone
	}
	buf := inm
	for {
		...
		if etagWeakMatch(etag, Headerget(w.Header(), "Etag")) {
			return condFalse
		}
		buf = remain
	}
...
func checkIfUnmodifiedSince(r *http.Request, modtime time.Time) condResult {
	ius := Headerget(r.Header, "If-Unmodified-Since")
...
func checkIfMatch(w http.ResponseWriter, r *http.Request) condResult {
	im := Headerget(r.Header, "If-Match")
	if im == "" {
		return condNone
	}
	for {
...
func MyFileServer(root http.FileSystem, s conf.Serve) http.Handler {
...
func checkIfModifiedSince(r *http.Request, modtime time.Time) condResult {
	...
	ims := Headerget(r.Header, "If-Modified-Since")
